...
export async function GET(request: Request) {
  ...
  if (!code) {
    ...
    return NextResponse.redirect(`${origin}/auth/auth-code-error?error=no_code&details=No_authorization_code_found_in_URL`);
  }
  ...
  try {
    ...
    return NextResponse.redirect(`${origin}/auth/auth-code-error?error=${encodeURIComponent(error?.message || 'unexpected')}`);
  }
} 